...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // Retrieve form data
    $duesID = $_GET['idToRemove'];

    // Validate required fields
    if ($duesID !== null) {
        // Prepare the SQL delete statement
        $sql = "DELETE FROM tbl_monthly_dues WHERE Dues_ID = ?";

        // Prepare the statement
        $stmt = $pdo->prepare($sql);

        // Bind parameters
        $stmt->bindParam(1, $duesID);
        ...
        if ($stmt->execute()) {
            ...
            // Prepare log details
            $action = 'Delete Monthly Dues Record';
            $description = "$user deleted monthly dues record with Dues_ID $duesID";
            $date = date('Y-m-d');
            $time = date('H:i:s');
